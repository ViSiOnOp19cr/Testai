...
/**
 * ProjectsPage Component - API Key Management
 * 
 * This page allows users to:
 * 1. View all their existing API keys
 * 2. Create new API keys (requires password confirmation)
 * 3. Copy API keys to clipboard
 * 
 * Features:
 * - Modal dialog for creating new keys
 * - Loading skeletons while fetching data
 * - Success notification when key is created
 * - Password verification for security
 */

// TypeScript interface for API key data
interface ApiKey {
  id: string
  apiKey: string
  prefix: string
    ...
    const fetchApiKeys = async () => {
      if (user?.email) {
        ...
        try {
          ...
          // Only log error if it wasn't caused by abort
          if (!abortController.signal.aborted) {
  ...
  return (
    <Layout>
      <div className="px-4 py-6 sm:px-0">
        ...
        {/* Success notification for newly created key */}
        {createdApiKey && (
